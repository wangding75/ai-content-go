...
每个迭代只要新增或改造前端页面，必须同时交付以下内容：

| 项目 | 要求 |
|---|---|
| 原型基准 | 以前端原型 `prototype/ai-content-factory-clickable-prototype.html` 的对应页面作为布局和交互基准 |
| 明确路由 | 迭代文档必须写明页面路由，不允许大量页面共用 `/` 作为占位路由 |
| 导航入口 | 系统级页面接入首页或全局侧边栏；项目级页面接入项目详情 / 项目工作区菜单 |
| 可访问性 | 不允许核心页面只能通过手输 URL 访问；刷新后可直接访问 |
| 交互闭环 | 原型中的核心按钮、筛选、详情、弹窗、状态反馈必须可点击并映射到 API |
| 测试覆盖 | e2e 或集成测试必须覆盖从入口点击进入页面、触发主要动作、展示成功态和错误态 |
