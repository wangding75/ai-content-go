...
func TestTask05MemoryAPIReturns409IdempotencyConflictEnvelope(t *testing.T) {
	rr1 := iteration6Request(t, http.MethodPost, "/api/v1/projects/project-1/consistency-reports", []byte(`{"range":{"latest":true},"scope":"project","severity_threshold":"low"}`))
	if rr1.Code != http.StatusAccepted {
		t.Fatalf("first create report = %d, want 202 body=%s", rr1.Code, rr1.Body.String())
	}
	rr2 := iteration6Request(t, http.MethodPost, "/api/v1/projects/project-1/consistency-reports", []byte(`{"range":{"latest":true},"scope":"project","severity_threshold":"high"}`))
