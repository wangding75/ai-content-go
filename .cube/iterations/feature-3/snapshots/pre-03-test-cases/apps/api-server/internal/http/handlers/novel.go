...
func (h *NovelHandler) CreatePlanningRun(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		_, _ = h.workflow.CancelRun(r.Context(), wf.WorkflowRunID, workflow.CancelRunRequest{Reason: "planning run creation failed"}, key)
...
func parseNovelPagination(r *http.Request) (content.PaginationRequest, error) {
	...
	return content.PaginationRequest{Page: page, PageSize: pageSize, Sort: r.URL.Query().Get("sort"), Order: r.URL.Query().Get("order")}, nil
}
