...
// @Test
func TestTask02CreateGenerationRunPersistsPendingRunAndPlannedContentThenReconcilesToPendingReview(t *testing.T) {
	svc := NewService()
	...
	if detail.ContentItems[0].Status != ContentItemPlanned || detail.ContentItems[0].GenerationRunID != created.GenerationRunID {
		t.Fatalf("expected accepted content to start as planned, got %#v", detail.ContentItems[0])
	}
	if err := svc.ReconcileWorkflowResult(context.Background(), created.WorkflowRunID); err != nil {
		t.Fatalf("reconcile workflow result: %v", err)
	}
	detail, err = svc.GetGenerationRun(context.Background(), created.GenerationRunID)
	if err != nil {
		t.Fatalf("get generation run after reconcile: %v", err)
	}
	if detail.Status != GenerationRunSucceeded || detail.ContentItems[0].Status != ContentItemPendingReview {
		t.Fatalf("expected successful reconciliation to move run and content to review, got %#v", detail)
	}
}
