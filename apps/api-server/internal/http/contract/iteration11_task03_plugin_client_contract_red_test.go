...
func TestTask03PluginClientAndAuthHTTPExposeOneTimeKeyAndNoAdminBearerRequirement(t *testing.T) {
	...
	activeAPIKey, _ = rotated["api_key_once"].(string)

	req := httptest.NewRequest(http.MethodPost, "/api/v1/plugin-auth/token", bytes.NewReader([]byte(`{"api_key":"`+activeAPIKey+`","client_version":"1.0.0"}`)))
	...
	disabledAuthReq := httptest.NewRequest(http.MethodPost, "/api/v1/plugin-auth/token", bytes.NewReader([]byte(`{"api_key":"`+activeAPIKey+`","client_version":"1.0.0"}`)))
