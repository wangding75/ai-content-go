...
func TestEngineWorkerTransitionsRunToRunningStatus(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		if len(mock.updateRunStatusCalls) > 0 {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}

	if len(mock.updateRunStatusCalls) == 0 || mock.updateRunStatusCalls[0] != "running" {
		t.Fatalf("expected engine to call UpdateRunStatus(running), got calls: %v", mock.updateRunStatusCalls)
	}
}
