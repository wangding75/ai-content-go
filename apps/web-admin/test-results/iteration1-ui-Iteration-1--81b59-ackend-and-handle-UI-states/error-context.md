...
- Name: iteration1-ui.spec.ts >> Iteration 1 Web Admin frontend UI roundtrip >> home dashboard, projects, detail shell, and content templates call real backend and handle UI states
- Location: e2e/iteration1-ui.spec.ts:18:7
...
- generic [ref=e1]:
  - navigation "Iteration 2 navigation" [ref=e2]:
    ...
    - link "LLM Logs" [ref=e9] [cursor=pointer]:
      - /url: /llm/logs
  - main [ref=e10]:
    - status [ref=e11]: 创建成功：project-new
    - generic [ref=e12]:
      - heading "项目管理" [level=1] [ref=e13]
      - generic [ref=e14]:
        - text: 项目名称
        - textbox "项目名称" [ref=e15]: RED roundtrip project
      - generic [ref=e16]:
        - text: 项目模板
        - combobox "项目模板" [ref=e17]:
          - option "Blog"
          - option "Article Pack" [selected]
      - button "提交新建项目" [active] [ref=e18]
      - paragraph [ref=e19]: 空状态：暂无项目
      - list
      - button "进入项目" [ref=e20]
...
```ts
  ...
  2   | 
  3   | const apiBaseURL = process.env.API_BASE_URL ?? 'http://127.0.0.1:18081';
  4   | const webBaseURL = process.env.WEB_BASE_URL ?? 'http://127.0.0.1:3001';
  5   | 
  6   | const requiredEndpoints = [
  7   |   '/api/v1/dashboard/summary',
  8   |   '/api/v1/projects',
  9   |   '/api/v1/projects/seed-project/overview',
  10  |   '/api/v1/projects/seed-project/pause',
  11  |   '/api/v1/content-types',
  12  |   '/api/v1/content-types/seed-content-type/project-schema',
  13  |   '/api/v1/prompt-templates',
  14  |   '/api/v1/llm-providers',
  15  | ];
  16  | 
  17  | test.describe('Iteration 1 Web Admin frontend UI roundtrip', () => {
  18  |   test('home dashboard, projects, detail shell, and content templates call real backend and handle UI states', async ({ page }) => {
  19  |     const apiHits: string[] = [];
  20  |     page.on('response', (response) => {
  21  |       const url = new URL(response.url());
  22  |       if (url.pathname.startsWith('/api/v1/')) {
  23  |         apiHits.push(url.pathname);
  24  |       }
  25  |     });
  26  | 
  27  |     await page.goto(webBaseURL, { waitUntil: 'domcontentloaded' });
  28  | 
  29  |     await expect(page.getByRole('heading', { name: '首页 / 系统大盘' })).toBeVisible();
  30  |     await expect(page.getByTestId('dashboard-loading')).toBeVisible();
